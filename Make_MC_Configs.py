...
## Fully hadronic GEN-SIM production
#step = "GEN-SIM"
#nEvents = 50000
#jobs_jobsize = 200
#masses = [250]
...
prevStep_prefix = "GEN-SIM" # if step == DR1: this = GEN-SIM. if step == DR2 with pileup: this = wPU_DR1 
nEvents = 50000 
jobs_jobsize = 1 
directory_prefix = "/eos/uscms/store/user/rasharma/double-higgs/SignalSample/" # location of GEN-SIM, DR1 or DR2 output files 
pileup = "wPU"
masses = ['X250']
  ...
  for dir in dir_list:
    ...
    if dir_mass in masses and dir_fstate in finalStates:
      ...
      for sndDirec in os.listdir(direc):
        ...
        print 'check: ',str(nEvents) + 'events_' + prevStep_prefix
        if str(sndDirec) == str(nEvents) + 'events_' + prevStep_prefix + '_CrabConfig': 
        	direc += '/'
        	direc += (sndDirec)
        	print'direc = ',direc 
...
## DR1, DR2, or MINIAOD step 
  ...
  for idd,directory in enumerate(directories):
     print idd, directory
     MC_Configs_Entry = '''
     { 
     		"step"      : "{step}",
     ...
     if idd is not len(directories)-1: MC_Configs += ',' # need comma separation 
     else: continue # no comma at end of last object 
